...
新增可独立运行的 **4S 店展车分析模块**：人工上传平面图、标定摄像头与展车区域，消费在线追踪结果，分析区域停留与回访，按自然日生成 HTML／JSON 日报。启动及使用方法见第 8 节。当前为可联调的第一版，不等同于已通过门店现场验收。
...
左侧纵向导航依次为“实时在线”“离线追踪”“展车分析”，当前模块高亮，右侧显示对应内容。在线／离线在当前工作台切换并保留已填写的流地址和已选文件；展车入口切换到独立服务，展车页面也提供相同的左侧导航。切换模块不会停止后台任务。可通过 `/?mode=online` 或 `/?mode=offline` 直接进入对应追踪模块。未配置展车服务地址时，展车入口仍显示但不可用，配置方式见第 8 节。
...
3. 在“追踪参数”中选择“离线并行路数”（默认 2，范围 1–4），再点击“开始离线追踪”。页面先显示逐文件上传进度，然后显示模型加载、并行逐帧追踪、统一身份融合和并行视频输出阶段。
...
离线 Web/API 采用**有界视频并行**：默认最多 2 路同时运行，可选择 1–4 路；例如选择 2 路并输入 5 个视频，先处理前两路，有空闲工作线程时再处理后续文件。单个视频内部始终按顺序读帧，不抽帧、不按实时速度丢弃积压；视频探测和上传仍依次执行。

每个推理工作线程在自己的线程内创建并持有独立 YOLO 和 TransReID 模型，处理下一段视频时复用；每个视频有独立的跟踪器、解码器、特征库和临时记录。由于 TransReID 初始化涉及共享模块导入路径，模型初始化受锁保护，但实际检测和 ReID 推理不共用一把锁。并行会增加模型内存/显存占用，不保证并行路数越多就越快；显存不足或 CPU 资源紧张时选 1 路或减小特征提取批大小。[Ultralytics 模型线程安全说明](https://docs.ultralytics.com/guides/yolo-thread-safe-inference/)

流程分两遍读取，每个工作线程只顺序解码当前文件，不缓存整段视频或全部人体裁剪：第一遍多路并行检测、跟踪、抽样保存外观特征并写逐帧记录；所有视频完成后，按输入视频顺序和路内确认顺序分配内部轨迹编号，再统一融合全局 ID，避免完成先后改变 ID。融合后第二遍多路并行生成标注结果，各路独立写日志，最后按摄像头/帧顺序合并。每条轨迹最多保留 64 个均匀蓄水池抽样特征，融合代表库最多 32 个；内存仍随并行模型数、分辨率和轨迹数量增加。

任一路失败会取消尚未启动的视频，并让其他工作线程在检查点停止，保留原始错误；用户取消同样作用于整批任务。帧数与时间戳校验、原生 MP4 和逐帧回放机制保持不变。此并行参数仅用于 Web/API 离线处理，原有 `demo.py` 命令行流程不受影响。
...
- 一个服务同时运行一个追踪任务，在线与离线共用该限制。再次提交会返回 HTTP 409；这不限制同一个离线任务内通过“离线并行路数”控制的视频并行。
...
curl -X POST http://127.0.0.1:8765/api/jobs/offline \
  ...
  -d '{"upload_ids":["第一个上传ID","第二个上传ID"],"options":{"confidence":0.35,"reid_threshold":0.30,"batch_size":4,"min_reid_frames":10,"offline_workers":2}}'
```
...
| `offline_workers` | 2 | 离线同时处理的视频数，整数 1–4；实际不超过输入文件数，超出路数的文件排队；1 为串行，不影响在线模式 |
...
HTTP 服务默认单 worker，并对数据目录加进程锁；不要使用多个 Uvicorn worker 启动共享同一个目录。离线视频并行在线程池中完成，仍在同一个可整体终止的任务子进程内，不会另外遗留视频子进程。流地址只在任务内存中使用，不写入任务 JSON，也不会回显到参数验证错误；第三方解码器的原始输出在推理子进程中被屏蔽，以避免泄露流密码。对外错误只返回经过处理的信息。
...
离线并行回归额外验证实际推理/编码重叠执行、模型线程隔离与复用、并行度上限、串行/并行的 ID 和日志及逐帧画面一致性、可变时间戳，以及取消/单路失败后的资源释放。
...
```text
serve.py                 # Web 启动入口
serve_showroom.py         # 独立展车服务入口，默认 8766
tracking_contracts/      # 通用、版本化追踪事件；无展车业务依赖
ui/                      # 两个服务共用的左侧导航样式，不含业务逻辑
modules/showroom/         # 展车专属配置、存储、统计、调度、API 与页面
web/
...
```text
在线追踪进程（唯一模型实现、内存特征）
    → tracking_contracts：不含特征向量的帧观察与健康事件
    → 追踪服务只读 HTTP 事件接口
    → modules/showroom：独立消费、标定映射、区域停留、日报
```

展车模块不导入追踪私有类、不复制模型、不读取追踪数据目录、不加载 PyTorch；只有只读 HTTP 客户端。升级追踪算法后，只要保持事件协议兼容，展车模块就会接收新的追踪结果。业务统计准确性仍需现场验证。

**特征向量仍只保存在追踪进程内存。** 展车 SQLite 保存的是门店、布局、位置与停留区间、消费游标、统计、人工角色和日报，不保存外观特征。报告不包含特征向量。追踪重启后不能恢复原有身份；统计身份键为 `run_id + global_id`，跨运行可能重复计人，报告会提示。
...
```bash
python -m pip install -r requirements-showroom.txt
```

终端一启动原追踪工作台，并显示展车入口：

```bash
python serve.py --showroom-url http://127.0.0.1:8766
```

终端二启动展车服务：

```bash
python serve_showroom.py --tracking-url http://127.0.0.1:8765
```

- 追踪工作台：[http://127.0.0.1:8765](http://127.0.0.1:8765/)
- 展车分析：[http://127.0.0.1:8766](http://127.0.0.1:8766/)
- 展车接口文档：[http://127.0.0.1:8766/docs](http://127.0.0.1:8766/docs)

两个服务可以独立启停。停止展车服务不停止追踪；再次启动后从持久化游标补读。仅使用追踪时，照旧执行 `python serve.py`，无需运行展车服务。每个数据目录只允许单进程、单 worker；不能用多个 Uvicorn worker 共享此 SQLite 目录。

若分析部署在另一台机器，可单独安装 `requirements-showroom.txt`，无需下载模型。`--tracking-url` 是分析服务器访问追踪 API 的地址，`--showroom-url` 是用户浏览器访问展车页面的地址，两者不要混淆。

### 页面配置流程

1. **创建门店**：填写名称、时区、营业时间和有效停留阈值。默认 `Asia/Shanghai`，09:00–18:00，停留阈值 10 秒。本版营业时段必须在同一天内。
2. **上传平面图**：接受 PNG／JPEG，最多 8 MiB、1200 万像素；服务校验并去除元数据后保存 PNG。填写图片所覆盖矩形的实际宽高（米），去掉图外页边留白。坐标原点在左上，右为 X，下为 Y。
3. **绘制展车区域**：填写稳定的车辆编号、名称、车型，在平面图点选至少三点形成关注区域。应覆盖车辆周围可站立的位置，不能只圈车身。支持继续添加多个展车；重叠区域不会强行归属。
4. **摄像头标定**：先在实时概览选择在线任务，再读取摄像头画面；也可上传截图。输入摄像头编号及原视频宽高。每组先点摄像头画面中的地面点，再点平面图对应位置，至少四组分散、非共线的点；点击“保存摄像头标定”，可继续添加其他摄像头。预览缩放会换算回原视频坐标，手工截图禁止裁剪。
5. **发布布局**：确认生效时间。首次配置可早于任务开始；已有数据时只允许对尚未处理的未来数据生效。发布时检查多边形与标定有效性。拟合误差不是现场定位精度，四点标定必须额外用独立测点验收。
6. **绑定在线任务**：核对各任务摄像头编号（按流输入顺序从 0 开始）。绑定后持续补读日志，实时图显示最近 10 秒的观察。新追踪任务需重新确认并绑定，不自动套用身份。暂停／恢复只影响分析消费者。
7. **查看日报**：选择门店日期生成当前草稿或补算历史日报，查看／下载 HTML 及同版本 JSON。当前人员可标记为员工或访客；角色修改后重新生成相关日报。

配置支持导出／导入 JSON；图片 ID 是当前门店的上传资产 ID，跨服务搬迁配置需先上传平面图并替换 ID。日报中的布局带生效版本，不覆盖历史布局。

### 统计口径与每日调度

- 用人员框底边中点近似脚点，通过地面单应变换映射平面图。只在标定控制点凸包内归属车辆；图像尺寸变化、脚点截断、无可靠特征质量、未标定或区域重叠记为未知。
- 只累计连续帧、同一身份、同一车区的已观测时间。空帧、跳帧和超出 `max_gap`（默认 2 秒）的间隔不补计停留。近邻片段可归为同一访问段，但其中断档单列为未知秒数，不加到有效秒数。
- 同运行同 GID 的多镜头、同车区停留取时间并集，避免重复计时；同一身份同时归属不同展车的冲突区间全部剔除。回访需要达到停留阈值、观察到离开，并满足默认 15 秒离开间隔。
- 日报包括全部配置展车（含零观察展车）、车型、人员会话、停留、中位时长、回访、逐次访问明细、小时分布、平面图区域、摄像头覆盖和缺失说明。没有观测的小时标记未知，不当作零客流。车辆区域停留不等于视线或购买意愿。
- 按门店时区自然日 `[00:00, 次日00:00)` 截断跨日停留。营业时间只用于覆盖率分母，日报仍包含当天全部已收到数据。展示的是访问片段，不是经过入口验证的新到店次数。
- 服务运行时，**当地时间 00:10 后**生成昨日报告，并每分钟检查近 30 天自门店创建以来的日期，补算未生成／有数据变化的日报。停机期间不运行调度，重启后补算；更早日期可手动生成。不会创建系统 cron，也不会发送外部通知。
- HTML 与 JSON 在同一数据库事务、同一报告修订中保存；无输入变化不重复增版，有迟到数据、角色变化或质量变化则产生新修订。下载接口返回该日期最新修订。今日标记草稿，过去日期分为已完成、数据不完整、无有效数据；“已完成”不是准确率保证。

### 数据与运行配置

| 配置 | 默认值 / 作用 |
| --- | --- |
| `MTMC_SHOWROOM_DATA_DIR` | `web_data/showroom`，独立业务 SQLite 与上传平面图 |
| `MTMC_TRACKING_URL` | `http://127.0.0.1:8765`，只读上游，可用启动参数覆盖 |
| `MTMC_SHOWROOM_URL` | 不设置时原工作台不显示展车入口，可用 `serve.py --showroom-url` 设置 |
| `MTMC_SHOWROOM_ALLOWED_HOSTS` | `127.0.0.1,localhost,::1,testserver`；内网部署需指定实际访问主机 |
| `serve_showroom.py --host / --port` | 默认 `127.0.0.1 / 8766` |

业务库为 `web_data/showroom/showroom.sqlite3`，平面图在其下的 `floorplans/`。在线事件日志由追踪服务拥有，位于各任务的 `events/`，按 16 MiB 或小时分段，以已同步到磁盘的 manifest 暴露游标；空帧也输出事件。写入队列最多 2048 条且不超过 16 MiB，磁盘慢或队列满时记录事件丢弃数，**不会无限阻塞追踪，也不承诺日志零丢失**。相机本身未传到服务器的帧无法从本日志恢复。

本版不自动删除业务数据、图片、报告或追踪日志。需监控磁盘并制定保留期；删除源日志会使尚未消费的历史无法补读。备份时应先停止展车服务，再备份整个展车目录（含 SQLite 的 WAL／SHM，如存在）并保留相应追踪日志。不要在服务运行时仅复制主 `.sqlite3` 文件。正式保留删除及在线备份流程留待下一阶段，不会擅自清理原有项目数据。

### API 与扩展点

追踪事件接口：`GET /api/jobs/{job_id}/events?cursor=0:0&limit=250`。返回 `schema_version=1`、普通观察、下一游标、是否读到已保存日志末尾、丢弃数及上游状态；不输出向量或视频源地址。**旧任务没有此日志，需要升级后新建在线任务**。当前命令行 `demo_stream.py` 不提供此 Web 事件接口。

展车 API 前缀：`/api/showroom/v1`。写请求需要 `X-MTMC-Client: web`，浏览器写入需同源。

| 路径 | 方法与用途 |
| --- | --- |
| `/stores` | GET 门店列表；POST 创建门店 |
| `/stores/{id}/floorplans` | POST 原始 PNG／JPEG 二进制 |
| `/stores/{id}/layouts` | GET 版本；POST 发布标定、车辆配置 |
| `/tracking/jobs` | GET 上游在线任务列表 |
| `/stores/{id}/bindings` | GET 绑定列表；POST 绑定在线任务 |
| `/stores/{id}/bindings/{run}/pause` | POST `{"paused":true/false}` |
| `/stores/{id}/live` | GET 最近位置与消费状态 |
| `/stores/{id}/roles` | POST 人工标记 `run_id / global_id / role` |
| `/stores/{id}/reports` | GET 最近最多 366 个日期的报告索引 |
| `/stores/{id}/reports/{YYYY-MM-DD}` | POST 生成／更新日报 |
| `/stores/{id}/reports/{YYYY-MM-DD}.html` 或 `.json` | GET 最新修订；`?download=true` 下载附件 |

模块目录内：`client.py` 管上游边界，`repository.py` 管业务存储与幂等消费，`geometry.py` 管几何映射，`reports.py` 管业务统计和报告，`service.py` 管消费与日调度，`app.py` 与 `static/` 管接口和页面。未来替换业务存储不必修改追踪模型；特征数据库适配属于未来追踪侧工作，本版没有实现。

### 已验证及仍需现场完成

专项测试执行：`python -m unittest discover -s tests -p test_showroom.py -v`。覆盖事件轮转与游标、实际 HTTP 协议联通、原子回滚、去重、断帧、跨车冲突、暂停续算、员工排除、日报修订、跨午夜及夏令时、每日补算、上传与同源边界；不加载模型。

本版仅接入 Web 在线任务；离线视频缺少绝对录制起始时间，暂不接入按日展车统计。尚未实现跨运行身份恢复、员工自动识别、视线／开门／坐入识别、录像证据索引、历史重新标定回放、生产认证权限和自动数据清理。默认接收时间也不是硬件同步时间。上线前需完成实店标定、独立标注准确率验收、容量与长时间故障测试，以及访问控制、备份和现场告知安排。**不要直接暴露在公网。**
